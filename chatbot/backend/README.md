...
```bash
# The unified .env file is in the project root directory
...
Required variables (see `.env.example` at the project root for full list):
```bash
# Database
DB_HOST=localhost
DB_PORT=5432
...
**Note:** The `.env` file should be at the project root, not in the backend directory.
