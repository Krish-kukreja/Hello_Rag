...
# Get the project root directory (2 levels up from start.sh)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../" && pwd)"
ENV_FILE="$PROJECT_ROOT/.env"

# Load environment variables from unified .env at project root
if [ -f "$ENV_FILE" ]; then
    echo " Loading .env from: $ENV_FILE"
    export $(cat "$ENV_FILE" | grep -v '^#' | xargs)
else
    echo " .env file not found at: $ENV_FILE"
    echo " Please create one from: $PROJECT_ROOT/.env.example"
    exit 1
fi
