...
# With database upload (requires .env file at the project root)
...
**Note:** The `--database` flag requires database credentials in the unified `.env` file at the project root (see main README for setup).
...
**Note:** The scraper automatically cleans all pages. By default, it creates only a JSON file containing both `text` (raw wikitext) and `cleaned_text` (cleaned markdown) fields. Use `--text` flag to also export a separate raw wikitext file. Use `--database` flag to upload to PostgreSQL (requires `.env` file at the project root).
